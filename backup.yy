...
stmt_list: stmt ';' { std::cout<<"minisql > ";}
  | stmt_list stmt ';' {std::cout<<"minisql > ";}
  | QUIT{std::cout<<"quit"<<std::endl;return 0;}
  ;

stmt: select_stmt { API::select_value(driver.stmt);driver.Clear(4);};

select_stmt: SELECT  select_expr_list{ driver.emit("SELECTNODATA"); } ;
...
column_list: NAME { driver.emit("COLUMN4");std::cout<<$1<<std::endl; }
  | column_list ',' NAME  { driver.emit("COLUMN3"); $$ = $1 + 1; }
  ;

select_expr_list: select_expr { $$ = 1;driver.emit("COLUMN2"); }
    | select_expr_list ',' select_expr {$$ = $1 + 1;driver.emit("COLUMN1"); }
    | '*' { driver.emit("SELECTALL"); $$ = 1;driver.selectAll(); }
    ;
...
table_factor:
    NAME {driver.emit("TABLE");driver.addTableName($1);}


   /* statements: delete statement */

stmt: delete_stmt { driver.emit("delete STMT"); API::delete_Value(driver.stmt);driver.Clear(5);}
   ;

delete_stmt: DELETE delete_opts FROM NAME
    opt_where 
                  { driver.emit("DELETEONE");driver.addTableName($4); }
;
...
stmt: insert_stmt { driver.emit("STMT");API::insert_value(driver.stmt);driver.Clear(3); };
insert_stmt: INSERT insert_opts opt_into NAME
     opt_col_names
     VALUES insert_vals_list
      { driver.emit("INSERTVALS ");driver.addTableName($4);};
...
opt_col_names: /* nil */
   | '(' column_list ')' { driver.emit("INSERTCOLS "); }
   ;

insert_vals_list: '(' insert_vals ')' { driver.emit("VALUES "); }
   | insert_vals_list ',' '(' insert_vals ')' { driver.emit("VALUES "); }
...
create_index_stmt: CREATE KEY NAME ON NAME
   '(' NAME ')' {driver.emit("INDEX");driver.addIndexParam($3,$5,$7);}
   ;

   /** drop index **/
stmt: drop_index_stmt {
    driver.emit("drop INDEX");
...
column_atts: /* nil */ { $$ = 0; }
    ...
    | column_atts NULLX
    | column_atts UNIQUE '(' column_list ')' {  $$ = $1 + 2; }
