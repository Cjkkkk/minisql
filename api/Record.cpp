...
RecordSet::RecordSet(string RecordFileName) {
	int i, s;
	this->Record_FileName = RecordFileName;
	...
	p += sizeof(int);
	memcpy(reinterpret_cast<char*>(&s), p, sizeof(int));			//FreeAddr数组的size()
	p += sizeof(int);
	for (i = 0; i < s; i++) {
		Addr New;
...
int RecordSet::FindRecord(Addr p, Record& R) {
	...
	if (p.BlockNum > this->Top.BlockNum || (p.BlockNum == this->Top.BlockNum && p.FileOff > this->Top.FileOff))
		return -1;					//地址不能超过文件最后一行记录的地址
	R.self = p;						//记录结构中包含自己的位置，方便传参
...
Addr RecordSet::InsertRecord(Record& R) {
	if (!this->FreeAddr.empty()) {
		...
		//终止循环时，有两种情况：找到了比Top小的地址，和FreeAddr找完了都没有
		if (Lessp(temp, this->Top.BlockNum, this->Top.FileOff)) {
			int not_important = this->FindRecord(temp, Q);
			Q = R;
			Q.RecordState = VALID;						//强调Q已经重新有效了
			Q.self = temp;
			this->SaveRecord(Q);
			return temp;
		}
	}
	//本来就空的情况和第一个if里面把FreeAddr掏空的情况都在下面处理
	if(this->FreeAddr.empty()) {
		Addp(this->Top);
		R.self = this->Top;
		R.RecordState = VALID;
...
void RecordSet::DeleteRecord(Addr p) {
	...
	if (Equalp(p, Top.BlockNum, Top.FileOff)) {
		...
		//跳出循环时分两种情况，找到了第一条VALID的记录，或者文件是空的
	}
	else {
		this->FreeAddr.push_back(p);				//变动回收数组
	}
	this->SaveRecord(R);
}
